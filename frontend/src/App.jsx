...
function navClass({ isActive }) {
  return [
    'relative px-3.5 py-2 text-sm font-semibold rounded-lg transition-all duration-150',
    isActive
...
function mobileNavClass({ isActive }) {
  return [
    'flex flex-1 flex-col items-center justify-center gap-1 py-2.5 text-[11px] font-bold tracking-wide uppercase transition-colors duration-150 min-h-[3.75rem]',
...
function UserPill({ email, onLogout }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span
        className="hidden xl:inline max-w-[160px] truncate text-xs font-medium text-ink-muted"
        title={email}
      >
        {email}
      </span>
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main
        className={`flex-1 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10 ${
          isAuthenticated ? 'pb-28 md:pb-12' : ''
        }`}
      >
        <Routes>
        ...
        >
          <div className="mx-auto max-w-sm flex">
            <NavLink to="/" end className={mobileNavClass}>
              {({ isActive }) => (
                <>
                  <div className={`relative flex items-center justify-center w-10 h-7 rounded-xl transition-all duration-150 ${
